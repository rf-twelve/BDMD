...
class PageMachinery extends Component
{
    ...
    {
        $data = Machinery::with('file_images')->find($id);
        $this->setFields($data);
        $this->showFormModal = true;
    }
    ...
    public function saveRecord()
    {
        ...
        if (isset($this->data_id)) {
            $model = Machinery::find($this->data_id);
            $model->update($valid);
        } else {
            $model = Machinery::create($valid);
        }
    ...
    {
        return to_route('machinery-view',['user_id'=>auth()->user()->id, 'id'=> $id]);
    }
    ...
    {
        Machinery::destroy($this->data_id);
    ...
    {
        return view('livewire.bgmd.page-machinery',[
            'records' => $this->rows
        ]);
    }
}
